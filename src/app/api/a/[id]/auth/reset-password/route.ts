...
export async function POST(
  ...
  try {
    ...
    assertApiRateLimit(request, "shop-agent-reset-password", 10);
    const body = await readBoundedJson(request, BODY_LIMIT_BYTES);
    const parsed = schema.parse(body);
    const store = getShopAgentStore();
    const agentId = await store.consumeResetToken(parsed.token);
    const agent = agentId ? await store.getById(agentId) : null;
    if (!agent || agent.draftId !== id) throw new InvalidShopAgentLinkError();
    await store.updatePassword(agent.id, parsed.password);
    await store.revokeAllSessions(agent.id);
