...
export async function POST(request: NextRequest) {
  try {
    ...
    const json = await readBoundedJson(request as unknown as Request, 25_000_000) as any;
    if (json.backupVersion !== 1 && json.backupVersion !== 2) throw new Error("Unsupported backup version");
    ...
    if (json.sessions || json.chat) {
      ...
      if (chatData.sessions) {
        await getChatStore().importUser(user.id, { sessions: chatData.sessions, memories: chatData.memories || [], memoryEnabled: chatData.memoryEnabled });
      }
    }
    if (json.studio?.drafts) {
      ...
      for (const d of json.studio.drafts) {
        try { await store.create(user, d.brief); } catch {}
      }
    }
