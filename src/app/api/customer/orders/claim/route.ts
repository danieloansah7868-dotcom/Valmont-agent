...
export async function POST(request: NextRequest) {
  try {
    ...
    // Orders can only be claimed on websites whose owner enabled customer
    // accounts; other websites keep pure guest orders.
    ...
    if (!draft || !customerAccountsEnabled(draft.brief)) {
      throw new InvalidOrderClaimError(
        "This website does not offer customer accounts, so orders cannot be linked.",
      );
    }
