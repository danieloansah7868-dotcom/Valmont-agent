...
export function AppShell({
  ...
  return (
    /*
      The rail offset travels as a custom property so it can be applied from
      `md` up only. It used to be an inline `left`/`paddingLeft`, which browsers
      apply at every width — on a phone, where the rail is display:none, that
      still shoved the header and the page 248px to the right and left roughly a
      third of a 390px screen usable.
    */
    <div
    ...
    >
      ...
      <header
        className={`fixed inset-x-0 top-0 z-50 flex h-16 items-center border-b border-line bg-ivory-50/95 px-4 backdrop-blur md:left-[var(--app-rail)] md:px-7 ${transitionClass}`}
      >
      ...
      */}
      ...
      <main
        id="main-content"
        className={`min-h-[calc(100dvh-4rem)] pt-16 md:min-h-dvh md:pl-[var(--app-rail)] ${transitionClass}`}
      >
        {children}
      </main>
    </div>
