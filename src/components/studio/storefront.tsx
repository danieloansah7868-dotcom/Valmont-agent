...
function Shop({
  ...
  const isBundleSite = category === "data-bundles";
  ...
  const deliveryEnabled = isBundleSite ? false : payments.delivery.enabled;
  const lines = useMemo(
    () =>
      Object.entries(cart)
  ...
  const grouped = useMemo(() => {
    if (!isBundleSite) return null;
    return groupBundlesByNetwork(items);
  }, [items, isBundleSite]);

  const [activeNetwork, setActiveNetwork] = useState<BundleNetworkId>("mtn");

  useEffect(() => {
    if (!isBundleSite || !grouped) return;
  ...
  // For bundle site, compute network mismatch warnings
  const networkWarnings = useMemo(() => {
    if (!isBundleSite) return [];
    if (!isValidGhanaMobile(phone)) return [];
  ...
  }, [isBundleSite, phone, lines]);

  if (isBundleSite && grouped) {
    const networksWithBundles = (
      Object.keys(grouped) as BundleNetworkId[]
    ...
    return (
      <section id="menu" className="mx-auto max-w-3xl px-4 pb-10 sm:px-6">
              ...
              >
                ...
                {qty === 0 ? (
                  <button
                    ...
                    className="mt-1 rounded-xl px-3 py-2 text-sm font-bold"
                    style={{ background: accent, color: primary }}
        ...
        {networksWithBundles.length === 0 && (
          <p className="mt-4 text-sm text-slate-600">No bundles yet.</p>
        )}
...
function BundleCartDrawer({
  ...
  lines: Array<{ item: CatalogItem; price: number; quantity: number }>;
            ...
            <>
                ...
                {lines.map((line) => (
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      {line.item.bundle?.dataMb ? (
                        <span className="ml-1 text-xs opacity-60">
                          ({formatDataMb(line.item.bundle.dataMb)}
                          {line.item.bundle.validity
                            ? ` • ${line.item.bundle.validity}`
                            : ""}
                          )
                        </span>
              ...
              {!canCheckout ? (
                ...
                >
                  ...
                  <button
                    type="submit"
                    disabled={busy || !method || Boolean(phoneError)}
