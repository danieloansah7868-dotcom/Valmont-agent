...
## Stage 7b agent wallet checkout — threat notes and invariants

Stage 7b lets wallet money leave via an HTTP request made by a logged-in
agent, and lets it come back via an owner refund. The threats and the
answers, keyed to the hard rules (`R1`–`R10` in the code comments):

| Threat                                                     | Answer                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Tampered price in the request body** (R1)                | The buy route accepts only item ids, quantities and the recipient's phone number; zod silently strips anything else. Every line is re-priced on the server from the shop's own catalogue at the shop's agent discount (`agentUnitPriceMinor`), with duplicate item ids merged before the caps apply. The UI panel merely displays the server-rendered price.                                                                                                                                                  |
| **Browser shows a richer wallet than the server has** (R2) | The balance is re-read from the store right before the buy (the session copy may be hours old), and the real guard is the conditional `balance_minor >= amount` UPDATE inside `purchase()` — a stale read can only cause a friendly 409, never an overdraw.                                                                                                                                                                                                                                                   |
| **Replayed or raced checkout** (R3)                        | Exactly one `purchase` entry per order: written, with the balance change, in ONE transaction keyed on `order_id`, hidden behind a partial unique index (`studio_shop_wallet_entries(order_id) WHERE kind = 'purchase'`, on both engines). A replay returns the existing entry with no change; a loser of a two-buy race hits the balance guard → 409 and its order is flipped to `payment_failed` — no wallet charge.                                                                                         |
| **paidAt manipulation** (R4)                               | The order passes through `OrdersStore.markPaid` (`wallet:<entry id>` reference) — the same single channel the payments webhook uses — BEFORE any dispatch. `create()` never writes `paid_at` anywhere, including for agent orders.                                                                                                                                                                                                                                                                            |
| **Refusal side effects** (R5)                              | Gating, shop-off, recipient validation, unknown item, paused bundle, caps, GHS 5,000 ceiling, balance and the live-delivery guard are all evaluated before the order row exists.                                                                                                                                                                                                                                                                                                                              |
| **Provider double-send on retries**                        | The same delivery engine as public orders (I1–I6: claim-before-send, idempotent rows keyed `(order_id, line_index, unit_index)`). Test mode dispatches to the simulator; live needs the same per-shop verified/manual answer checkout requires.                                                                                                                                                                                                                                                               |
| **Wallet refunds** (R8)                                    | Owner-role only — a member holding every permission box still gets `ShopOwnerOnlyError`. Only an `agent_wallet` order with its purchase entry can be refunded (otherwise `AgentOrderNotRefundableError`, 409); the refund amount is the purchase entry. The credit lands once per order (same partial unique index on `kind = 'refund'`; a race maps to `WalletAlreadyRefundedError`), the status moves via `updateStatus → refunded`, and the ledger stays append-only — no entry is ever edited or deleted. |
| **Cross-agent or cross-owner visibility** (R9)             | Agent pages and the route read by `agent_id` in the SQL itself; another agent's order is a 404, as is an order fetched with a session from a sibling shop. The 32-hex access code is server-generated, never returned by the buy route and never rendered on agent pages — the agent gets no payment or proof link.                                                                                                                                                                                           |
| **Existing surfaces** (R10)                                | `agent_wallet` is NOT in `PAYMENT_METHODS` and never selectable in Studio → Payments or on the storefront; the public checkout refuses it exactly like any unknown method (400, no order, no payment link) and the webhook is untouched. Owner monitoring arrives through the same merchant alert email, which gains exactly one extra line — "Paid from agent wallet" — and only for wallet orders.                                                                                                          |

Test coverage lives in `src/lib/shop-agent/store-purchase.test.ts`,
`src/lib/shop-agent/postgres-shop-agent-purchase.test.ts` (CI-only, against
the throwaway PostgreSQL), `src/app/api/a/agent-orders-route.test.ts`,
`src/app/api/manage/shop-agent-refund-route.test.ts`,
`src/app/api/studio/checkout-agent-wallet.test.ts`,
`src/lib/studio/orders-agent.test.ts`, `src/lib/shop-agent/orders.test.ts`,
`src/app/a/[id]/pages-orders.test.ts`,
`src/app/manage/[id]/pages-agent-orders.test.ts`, and end-to-end in
`tests/e2e/shop-agent-buy.spec.ts`.
